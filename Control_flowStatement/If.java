import java.util.Scanner;
public class If{
public static void main (String [] args)
{
	Scanner sc = new Scanner (System.in);
	int bjp = 0,cng = 0,ss = 0, app = 0,mns = 0,nota= 0;
	System.out.println("       Wellcome      ");
	System.out.println();
	System.out.print("Enter The Population : ");
	int population = sc.nextInt();
	
	for(int i =1 ; i<= population ; i++)
	{
		System.out.println();
		System.out.println("*** LIST OF PARTIES ***");
		System.out.println("1. BJP");
		System.out.println("2. CONGRESS");
		System.out.println("3. SHIV SENA");
		System.out.println("4. AAP");
		System.out.println("5. MNS");
		System.out.println("6. NOTA");
		System.out.println();
		System.out.print("Enter the option : ");
		int opt = sc.nextInt();
		if (opt >= 1 && opt <= 6){
		if (opt ==1 )
		{
			bjp++;
			System.out.println("ACHE DIN AYENGE");
		}
		if (opt ==2 )
		{
			cng++;
			System.out.println("BHARAT TODO");
		}
		if (opt ==3 )
		{
			ss++;
			System.out.println("HUM HAI ASLI SENA");
		}
		if (opt ==4 )
		{
			app++;
			System.out.println("MUJHE AZAD KARO");
		}
		if (opt ==5 )
		{
			mns++;
			System.out.println("JAI MAHARASHTRA");
		}
		if (opt ==6 )
		{
			nota++;
			System.out.println("YOU ARE EDUCATED PERSON");
		}
	}
	if(!(opt >= 1 && opt <= 6 )){
		i--;
		System.out.print("INVALID OPTION");
		
	}
	
	if (bjp >= cng && bjp>= ss&& bjp>= app &&bjp>= mns &&bjp>= nota )
	{
		System.out.println("BJP HAS WON THE ELECTION BY "+bjp+" Votes");
		return ;
	}
	if (cng >= bjp && cng>= ss && cng>= app && cng>= mns && cng >= nota )
	{
		System.out.println("CNG HAS WON THE ELECTION BY "+cng+" Votes");
	}
	if (ss >= bjp && ss>= cng && ss >= app && ss >= mns && ss >= nota )
	{
		System.out.println("SS HAS WON THE ELECTION BY "+ss+" Votes");
	}
	if (app >= bjp && app>= ss && app>= cng && app>= mns && app >= nota )
	{
		System.out.println("AAP HAS WON THE ELECTION BY "+app+" Votes");
	}
	if (mns >= bjp && mns>= ss && mns>= cng && mns>= app && mns >= nota )
	{
		System.out.println("MNS HAS WON THE ELECTION BY "+mns+" Votes");
	}
	if (nota >= bjp && nota>= ss && nota>= cng && nota>= mns && nota >= app )
	{
		System.out.println("NOTA HAS WON THE ELECTION BY "+nota+" Votes");
	}
}	
	
}
}